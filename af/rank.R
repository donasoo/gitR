...
afc$effect <- str_trim(str_replace_all(afc$effect, '<span.*</span>|<p>|</p><em>|</em>|聽|\\(.*\\)|<div>|</b>|&#13;|</strong>|</div>|\\n', ''))
rank.list <- str_split(afc$effect, '<br />')
rank.list <- sapply(rank.list, function(x) x[nchar(x)>10])

for( i in 1:length(rank.list)){
  rank.list[[i]] <- paste(afc$id[i], rank.list[[i]])
}

rank.v <- unlist(rank.list)

write.table(rank.v, 'rankv.txt', row.names = F)
