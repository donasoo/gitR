---
title: "mining with svm kernel:ANOVA RBF "
...
mplot <- function(test, pre, name)
{
  ...
  p
  ggsave(file=paste('m-', name, '对比图(svm).png'), plot=p, width=5, height=4, dpi=100);
  ggsave(file=paste('l-', name, '对比图(svm).png'), plot=p, width=9, height=7.5, dpi=100);
  
  cat(paste(name, '\n'));
  # 点图
  plot(pre ~ test$cpi, main=paste(name, '对比点图'), xlab='真值', ylab='预测值')
  abline(0, 1, lty = 2)
  dev.copy(png,filename=paste('m-', name, '对比点图(svm).png'), width=500, height=400, bg="white");
  dev.off();
  dev.copy(png,filename=paste('l-', name, '对比点图(svm).png'), width=1000, height=700, bg="white");
  dev.off();
  print(p)
}
```
...
# 做一次svm的函数
dosvm <- function(train, test, name)
{
  set.seed(3721)
  model.svm <- ksvm(cpi~., data=train, kernel='anovadot', type='eps-svr', epsilon=0.04, C=5);
  
  pre.svm <- predict(model.svm,  newdata=test);
  
  mplot(test, pre.svm, name);
   
  # 保存误差
  error <- regr.eval(test$cpi, pre.svm, train.y = train$cpi);

  cat('误差\n')
  print(error)
  
  out <- list('error'=error, 'predict'=pre.svm)
  return (out);
...
p <- p + scale_y_continuous(breaks = eps)
print(p)

cat('通过参数选择图可看出，C=6, epsilon=0.01 拟合误差相对较小\n')

ggsave(file='m-参数选择图(svm).png', plot=p, width=5, height=4, dpi=100);
ggsave(file='l-参数选择图(svm).png', plot=p, width=9, height=7.5, dpi=100);
write.csv(amx, file = 'param-svm.csv');
...
for (i in 1:len)
{
  ...
  out <- dosvm(train=pi.train, test=pi.test, name);
  le[i, ] <- out$error;
...
# 用训练集作为测试 
out <- dosvm(train=allpi[, sl], test=allpi[, c( -23)], '估计值与真值');
le[len+2, ] <- out$error;
```

#预测
```{r, echo=FALSE, warning=FALSE, error=FALSE, message=FALSE, comment=NULL}
outp <- dosvm(train=allpi[, sl], test=pi14[, c( -23)], '外推预测');
le[len+3, ] <- outp$error;
...
rr <- rr[, -1]
write.csv(rr, file = 'rr-svm.csv');

lrr <- melt(rr)
lrr$date <- paste(as.character(lrr[, 2]), as.character(lrr[, 1]))
write.csv(lrr, file = 'lrr-svm.csv');
```
