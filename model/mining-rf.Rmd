---
title: "Mining with RandomForest"
...
mplot <- function(test, pre, name)
{
  ...
  ggsave(file=paste('m-', name, '对比图(rf).png'), plot=p, width=5, height=4, dpi=100);
  ggsave(file=paste('l-', name, '对比图(rf).png'), plot=p, width=9, height=7.5, dpi=100);
  
  cat(paste(name, '\n'));
  # 点图
  plot(pre ~ test$cpi, main=paste(name, '对比点图'), xlab='真值', ylab='预测值')
  abline(0, 1, lty = 2)
  dev.copy(png,filename=paste('m-', name, '对比点图(rf).png'), width=500, height=400, bg="white");
  dev.off();
  dev.copy(png,filename=paste('l-', name, '对比点图(rf).png'), width=1000, height=700, bg="white");
  dev.off();
  print(p)
}
```
...
```{r, echo=FALSE, warning=FALSE, error=FALSE, message=FALSE}
# 做一次rf的函数
dorf <- function(train, test, name)
{
  set.seed(3721)
  model.rf <- randomForest(cpi ~ ., data =train, mtry=50, ntree=700);
  
  pre.rf <- predict(model.rf,  newdata=test);
  
  mplot(test, pre.rf, name);
   
  # 保存误差
  error <- regr.eval(test$cpi, pre.rf, train.y = train$cpi);
  cat('误差\n')
  print(error)
  
  out <- list('error'=error, 'predict'=pre.rf)
  return (out);
...
for (i in 1:(length(seq)))
{
  ...
  out <- dorf(train=pi.train, test=pi.test, name);
...
# 用训练集作为测试 
out <- dorf(train=allpi[, sl], test=allpi[, c( -23)], '估计值与真值');
le[len+2, ] <- out$error;
...
outp <- dorf(train=allpi[, sl], test=pi14[, c( -23)], '外推预测');
le[len+3, ] <- outp$error;
...
print(le)
write.csv(le, file = 'error-rf.csv');
```
...
print(result)
write.csv(result, file = 'result-rf.csv');
```
...
rr <- rr[, -1]
write.csv(rr, file = 'rr-rf.csv');

lrr <- melt(rr)
lrr$date <- paste(as.character(lrr[, 2]), as.character(lrr[, 1]))
write.csv(lrr, file = 'lrr-rf.csv');
```
