...
model5 <- ksvm(cpi~.,data=allpi,kernel='anovadot', type='eps-svr', epsilon=0.01, C=1)
pk5 <- predict(model5, allpi)
plot(pk5 ~allpi$cpi, main='anovadot')
e5 <- regr.eval(allpi$cpi, pk5, train.y = allpi$cpi);
p <- param(model5)
mt <- paste(format(e5[5], digits = 4), p[1], p[2])
mtext(mt, side = 3)
cat('anovadot')
print(e5)

# 使用Laplacian核函数，正确的分类
model6 <- ksvm(cpi~.,data=allpi,kernel='anovadot', type='eps-svr', epsilon=0.05, C=1)
pk6 <- predict(model6, allpi)
plot(pk6 ~allpi$cpi, main='Laplacian')
e6 <- regr.eval(allpi$cpi, pk6, train.y = allpi$cpi);
p <- param(model6)
mt <- paste(format(e6[5], digits = 4), p[1], p[2])
mtext(mt, side = 3)
cat('anovadot')
print(e6)
...
p <- param(model8)
mt <- paste(format(e8[5], digits = 4), p[1], p[2])
mtext(mt, side = 3)
cat('ANOVA RBF ')
print(e8)

# 使用用ANOVA RBF 核函数，正确的分类
model9 <- ksvm(cpi~.,data=allpi,kernel='anovadot', type='eps-svr', epsilon=0.02, C=1)
pk9 <- predict(model9, allpi)
plot(pk9 ~allpi$cpi, main='ANOVA RBF')
e9 <- regr.eval(allpi$cpi, pk9, train.y = allpi$cpi);
p <- param(model9)
mt <- paste(format(e9[5], digits = 4), p[1], p[2])
mtext(mt, side = 3)
cat('Spline')
print(e9)
